...
function createJoke (event) {
  ...
  let prompt =
    "Only say 1 joke at a time.  Please use the format of (question, why did the woman cry all night?) (answer, she just said goodbye to her youth.)";
  let apiUrl = `https://api.shecodes.io/ai/v1/generate?prompt=${prompt}&context=${context}&key=${apiKey}`;

  let joke = document.querySelector("#joke");
  joke.innerHTML = "Joke incoming!!!";
  //placed the axios response inside the function so it does not automatically run. We want the button click to request the joke.
